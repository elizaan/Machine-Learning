...
def train_with_zero_initialization(X_train, y_train, X_test, y_test, width, gamma_0, d, epochs, batch_size):
    print(f"\nTraining with hidden layer width={width} and zero initialization")
    
    # Initialize the network with zero weights
    layer_sizes = [X_train.shape[1], width, width, 1]
    nn = NeuralNetwork(layer_sizes)
    for layer in range(1, nn.num_layers):
        prev_size = nn.layer_sizes[layer - 1]
        curr_size = nn.layer_sizes[layer]
        nn.weights[layer] = np.zeros((prev_size + 1, curr_size))  # +1 for bias

    # Train with SGD
    train_error, test_error = train_sgd(nn, X_train, y_train, X_test, y_test,
                                        width, gamma_0, d, epochs, batch_size)

    print(f"Zero Initialization - Width={width}, Training Error={train_error:.4f}, Test Error={test_error:.4f}")
...
def main():
    ...
    for width in widths:
        ...
        zero_train_error, zero_test_error = train_with_zero_initialization(X_train, y_train, X_test, y_test,
                                                                           width, gamma_0, d, epochs, batch_size)
        zero_init_results.append((width, zero_train_error, zero_test_error))

    # Print final results
    print("\nFinal Results (Random Initialization):")
    for width, train_error, test_error in results:
        print(f"Width={width}, Train Error={train_error:.4f}, Test Error={test_error:.4f}")

    print("\nFinal Results (Zero Initialization):")
    for width, train_error, test_error in zero_init_results:
        print(f"Width={width}, Train Error={train_error:.4f}, Test Error={test_error:.4f}")
